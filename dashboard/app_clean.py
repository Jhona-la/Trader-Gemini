...
st.title("Trader Gemini - Live Dashboard 🚀")
...
def load_data():
    ...
    try:
        ...
        st.info("💡 Tip: Try deleting 'dashboard/data/status.csv' and restart the bot to create a fresh file.")
        return pd.DataFrame(), pd.DataFrame()
...
if not status_df.empty:
    ...
    with col3:
        st.metric(
            ...
            help="**Ganancias/Pérdidas confirmadas** de trades ya cerrados. Esto es dinero real ganado o perdido."
        )
    with col4:
        st.metric(
            "⏳ Unrealized PnL", 
            f"${latest['unrealized_pnl']:,.2f}",
            help="**Ganancias/Pérdidas flotantes** de posiciones todavía abiertas. Solo se confirman al cerrar."
        )
        
    # Positions Table
    st.subheader("📈 Current Positions")
    st.caption("💡 **Tip:** Click '❌ Close' to manually close a position and take profits/cut losses.")
    ...
    try:
        ...
        if pos_data:
            ...
            st.markdown("---")
            st.markdown("#### 🎯 Manual Position Control")
            st.caption("Cerrar manualmente una posición (útil para tomar ganancias o cortar pérdidas)")
            ...
            with col_close1:
                ...
                selected_symbol = st.selectbox(
                    ...
                    symbols_open,
                    help="Elige qué posición quieres cerrar manualmente"
                )
            ...
            with col_close2:
                ...
                if st.button("❌ Close Position", type="primary", use_container_width=True):
                    ...
                    try:
                        ...
                        st.success(f"✅ Close signal sent for {selected_symbol}! The bot will close it on the next cycle.")
                    except Exception as e:
    ...
    if not trades_df.empty:
        ...
        if not closed_trades.empty:
            ...
            if not pnl_changes.empty:
                b1, b2 = st.columns(2)
                b1.metric("🏆 Best Trade", f"${pnl_changes.max():.2f}", help="The single most profitable trade.")
                b2.metric("💀 Worst Trade", f"${pnl_changes.min():.2f}", help="The single biggest loss.")
...
st.info("💡 **Tip:** Refresh the page (F5) to update data. No auto-refresh to keep dashboard stable.")
