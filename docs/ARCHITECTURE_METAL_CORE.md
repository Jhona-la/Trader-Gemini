...
- **Zero-Pandas/No-Allocation**: Elimination of Pandas objects and `Decimal` in the hot path prevents GIL contention and heap fragmentation. Integers/Float64 are processed via `math_kernel.py`.

### III. Deterministic Risk Gates (FASE 12)
Risk validation is the last barrier before execution, now fully JIT-compiled.
- **RiskCache & IEEE 754**: Calculations like Kelly and CVaR are moved to Numba kernels for nanosecond-level resolution using Float64.
