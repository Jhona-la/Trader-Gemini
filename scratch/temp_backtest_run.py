...
# MOCK NOTIFIER TO PREVENT SMTP HANGS DURING BACKTEST
class MockNotifier:
    @classmethod
    def send_trade_notification(cls, *args, **kwargs): pass
    @classmethod
    def send_trade_open(cls, *args, **kwargs): pass
    @classmethod
    def send_system_startup(cls, *args, **kwargs): pass
    @classmethod
    def send_backtest_progress(cls, *args, **kwargs): pass
    @classmethod
    def send_strategy_leaderboard(cls, *args, **kwargs): pass
    @classmethod
    def send_backtest_complete(cls, *args, **kwargs): pass
    @classmethod
    def send_error_alert(cls, *args, **kwargs): pass
    @classmethod
    def send_system_outage(cls, *args, **kwargs): pass
    @classmethod
    def send_custom_alert(cls, *args, **kwargs): pass

# Override the global Notifier with our Mock
import utils.notifier
utils.notifier.Notifier = MockNotifier
...
def run_global_backtest(
    ...
    try:
        ...
        try:
            ...
            #   históricas, y el ML aprende a no depender de ellas en backtest.
        ...
        t_start = time.time()
        last_progress = 0
        total_epochs = min(data_provider.total_epochs, 500)  # Quick run limit
        ...
        while data_provider.continue_backtest:
            ...
            if epoch_count >= total_epochs:
                print(f"  🏁 Reached requested total_epochs limit ({total_epochs}). Stopping backtest.")
                break
            ...
            while not events_queue.empty():
                ...
                with open('scratch/progress.log', 'a') as f: f.write(f'Processing {event.type} for {getattr(event, "symbol", "None")}\\n')
                if event.type == EventType.MARKET:
                    market_events.append(event)
    
            # ── Phase B: Process Market Events (prices, exits, strategies) ──
            for event in market_events:
                with open('scratch/progress.log', 'a') as f: f.write(f'  -> Phase B for {event.symbol}\n')
                    ...
                    try:
                        ...
                        if _loss_count >= ML_LOSS_STREAK_LIMIT:
                            ...
                            if _shadow_wins < ML_SHADOW_WIN_REQUIRED:
                                ...
                                with open('scratch/progress.log', 'a') as f: f.write(f'      -> [SHADOW] _run_inference for {symbol} {strat.__class__.__name__}\n')
                                strat._run_inference()
                                with open('scratch/progress.log', 'a') as f: f.write(f'      <- [SHADOW] _run_inference done\n')
                        ...
                        from utils.quantum_telemetry import QuantumTimer
                        timer = QuantumTimer()
                        
                        with open('scratch/progress.log', 'a') as f: f.write(f'      -> _run_inference for {symbol} {strat.__class__.__name__}\n')
                        strat._run_inference()
                        with open('scratch/progress.log', 'a') as f: f.write(f'      <- _run_inference done\n')
                        
                        latency_str = timer.format_all()
                        logger.info(f"⚡ [HFT LATENCY] Inferencia {symbol} ({_horizon}): {latency_str}")
                    ...
                    try:
                        ...
                        for me in market_events:
                            try:
                                ...
                                traceback.print_exc(); logger.debug(f"Strategy {g_strat.strategy_id} error on {me.symbol}: {e}")
                    except Exception:
                        pass
