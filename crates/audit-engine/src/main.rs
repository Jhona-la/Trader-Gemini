...
use phase_runner::{AdaptiveTimer, Phase, PhaseExecutor, PhaseResult};
...
async fn main() {
    ...
    thread::spawn(move || {
        while let Ok(result) = result_rx.recv() {
            println!(
                "✅ [RESULT LOGGER] Phase {} completed in {} ms.",
                result.phase.to_str(),
                result.duration_ms
            );
    ...
    println!(
        "Phase [Alpha]: System Graph built. Nodes: {}",
        system_graph.graph.node_count()
    );
        ...
        let result = match current_phase {
            Phase::Beta => {
                ...
                if !collisions.is_empty() {
                    println!(
                        "⚠️ WARNING: Detected parameter collisions: {:?}",
                        collisions
                    );
                }
