...
pub enum TelemetryEvent {
    LatencyWarning { dns_ms: u64, ws_ms: u64 },
    TradeExecution { 
        coin_id: usize, 
        is_long: bool, 
        pnl: f64, 
    ...
    RegimeShift { coin_id: usize, old_regime: u8, new_regime: u8 },
    NanoForestPrediction { coin_id: usize, ml_prob: f64, threshold: f64, execution_us: u64 },
    MemoryDiagnostic { ram_used_mb: u64, total_alloc_mb: u64, cpu_cycles: u64 },
    ParityAlert { message: String, expected: f64, actual: f64, diff: f64 },
    UpdateCapital { real_capital: f64 },
}
        ...
        while let Ok(event) = rx.recv() {
            match event {
                TelemetryEvent::UpdateCapital { real_capital } => {
                    ...
                    println!("🏦 [TELEMETRÍA] Capital Compuesto Inicializado Dinámicamente a: {:.4} USD", compounding_capital);
                },
                TelemetryEvent::LatencyWarning { dns_ms, ws_ms } => {
                    println!("⚠️ [TELEMETRÍA RED] DNS: {}ms | WS: {}ms", dns_ms, ws_ms);
                },
                TelemetryEvent::TradeExecution { coin_id, is_long, pnl, fees, roi_pre_fees, roi_post_fees, trade_duration_ms, current_wr } => {
                    total_pnl += pnl;
                    total_fees += fees;
                    total_trades += 1;
                    ...
                    if total_trades % 10 == 0 || pnl.abs() > compounding_capital * 0.05 {
                        ...
                        let days_to_double = if projected_daily_pnl > 0.0 { compounding_capital / projected_daily_pnl } else { f64::INFINITY };
                        
                        println!("📊 [PROYECCIÓN INSTITUCIONAL] Trade #{} | Coin: {} | Dir: {}", total_trades, coin_id, if is_long { "LONG" } else { "SHORT" });
                        println!("   ➤ PnL Neto Trade: {:.4} | Fees: {:.4} | ROI Post-Fee: {:.4}%", pnl, fees, roi_post_fees);
                ...
                TelemetryEvent::RegimeShift { coin_id, old_regime, new_regime } => {
                    println!("🔄 [TELEMETRÍA RÉGIMEN] Coin: {} | Shift: {} -> {}", coin_id, old_regime, new_regime);
                },
                TelemetryEvent::NanoForestPrediction { coin_id: _, ml_prob: _, threshold: _, execution_us: _ } => {},
                TelemetryEvent::MemoryDiagnostic { ram_used_mb, total_alloc_mb, cpu_cycles } => {
                    println!("💾 [TELEMETRÍA RENDIMIENTO] RAM Used: {}MB | Alloc: {}MB | Asymptotic CPU Cycles: {}", ram_used_mb, total_alloc_mb, cpu_cycles);
                },
                TelemetryEvent::ParityAlert { message, expected, actual, diff } => {
                    println!("🚨 [PARIDAD FORENSE] {} | Esperado: {:.4} | Real: {:.4} | Dif: {:.6}", message, expected, actual, diff);
                },
            }
        }
    }
...
pub fn send_trade_execution(
    coin_id: usize, is_long: bool, pnl: f64, fees: f64, 
    roi_pre_fees: f64, roi_post_fees: f64, trade_duration_ms: u64, current_wr: f64
) {
    TELEMETRY.send(TelemetryEvent::TradeExecution { 
        coin_id, is_long, pnl, fees, roi_pre_fees, roi_post_fees, trade_duration_ms, current_wr 
    });
}

#[inline(always)]
pub fn send_ml_prediction(coin_id: usize, ml_prob: f64, threshold: f64, execution_us: u64) {
    TELEMETRY.send(TelemetryEvent::NanoForestPrediction { coin_id, ml_prob, threshold, execution_us });
}

#[inline(always)]
pub fn send_parity_alert(message: &str, expected: f64, actual: f64, diff: f64) {
    TELEMETRY.send(TelemetryEvent::ParityAlert { 
        message: message.to_string(), expected, actual, diff 
    });
}
