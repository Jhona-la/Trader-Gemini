...
impl VolatileMomentumBooster {
    ...
    pub fn calculate_tp_extension(
        ...
        if !raw_pnl_pct.is_finite() || !hawkes_ratio.is_finite() || !atr_pct.is_finite() || !position_direction.is_finite() {
            return 1.0;
        }
        use std::sync::atomic::Ordering;
        // FIX #662: Sanitizar atr_pct y normalizar dirección de posición
        let safe_atr = if atr_pct.is_finite() && atr_pct > 0.0 { atr_pct } else { 0.001 };
        let pos_dir = if position_direction > 0.0 { 1.0 } else if position_direction < 0.0 { -1.0 } else { 0.0 };
        ...
        let alignment_activation = ((momentum_alignment - alignment_threshold) * 5.0).tanh().max(0.0);
        
        // FIX #603: Acotar hawkes_ratio y base_weight para evitar saturación espuria ante picos de volatilidad
        let hw = arena.config.tensor_poly_a.load(Ordering::Relaxed).clamp(0.0, 5.0);
        let pw = arena.config.tensor_poly_b.load(Ordering::Relaxed).clamp(0.0, 5.0);
        ...
        let max_boost = arena.config.explosive_leverage_multiplier.load(Ordering::Relaxed).clamp(1.0, 10.0);
        let boost_factor = 1.0 + (alignment_activation * base_weight).tanh() * (max_boost - 1.0);
        
        if boost_factor.is_finite() { boost_factor.max(1.0) } else { 1.0 }
    }
}
...
mod tests {
    ...
    fn test_momentum_booster_strong_aligned_momentum_expansion() {
        ...
        assert!(boost >= 1.0, "Momentum fuertemente alineado debe producir boost >= 1.0");
    ...
    fn test_momentum_booster_nan_and_negative_inputs_immunity() {
        let arena = quantum_arena::GlobalArena::new(13.0);
        let boost = VolatileMomentumBooster::calculate_tp_extension(f64::NAN, f64::NAN, f64::NAN, f64::NAN, &arena);
