...
impl SwingEngine {
    ...
    #[inline(always)]
    pub fn evaluate_trend(&mut self, price: f64, hurst: f64, trend_threshold: f64, ml_pred: f64, arena: &quantum_arena::GlobalArena) -> SignalIntent {
        telemetry_server::profile_node!("SwingEngine::evaluate_trend", {
            ...
            if hurst < trend_threshold {
                let conf_fallback = arena.config.explosive_confidence_threshold.load(Ordering::Relaxed);
                if z_score > z_thresh || ml_pred < (0.5 - ml_short) {
                    let conf = if ml_pred < (0.5 - ml_short) { (0.5 - ml_pred) * 2.0 } else { conf_fallback };
                    return SignalIntent {
                        signal: SignalType::Short, // Sobrecomprado, vender
                        confidence: conf,
                        expected_duration_ms: (arena.config.base_duration_ms.load(Ordering::Relaxed) * 60.0) as u64,
                        horizon: crate::TradeHorizon::Swing,
                        ..Default::default()
                    };
                } else if z_score < -z_thresh || ml_pred > (0.5 + ml_long) {
                    let conf = if ml_pred > (0.5 + ml_long) { (ml_pred - 0.5) * 2.0 } else { conf_fallback };
                    return SignalIntent {
                ...
                if macd_diff > threshold || ml_pred > (0.5 + ml_long) {
                    return SignalIntent {
                        ...
                        confidence: ((macd_diff.abs() * hurst * 100.0).max((ml_pred - 0.5) * 2.0)).tanh(),
                        expected_duration_ms: (arena.config.base_duration_ms.load(Ordering::Relaxed) * 60.0) as u64,
                        horizon: crate::TradeHorizon::Swing,
                        ..Default::default()
                    };
                    ...
                    return SignalIntent {
                        ...
                        confidence: ((macd_diff.abs() * hurst * 100.0).max((0.5 - ml_pred) * 2.0)).tanh(),
                        expected_duration_ms: (arena.config.base_duration_ms.load(Ordering::Relaxed) * 60.0) as u64,
                        horizon: crate::TradeHorizon::Swing,
                        ..Default::default()
                    };
