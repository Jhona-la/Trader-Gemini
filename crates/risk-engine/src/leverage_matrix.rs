...
impl QuantumLeverageMatrix {
    ...
    pub fn calculate_dynamic_leverage(
        ...
        let fraction_multiplier = if prob_win > arena.config.veto_threshold_btc.load(Ordering::Relaxed) { 
            1.0
        } else {
        ...
        let log_divisor = arena.config.lev_matrix_log_cap_divisor.load(Ordering::Relaxed);
        ...
        let growth_scalar = arena.config.lev_matrix_growth_scalar.load(Ordering::Relaxed);
        ...
        let vol_clamp_min = arena.config.lev_matrix_vol_clamp_min.load(Ordering::Relaxed);
        ...
        let (final_vol_factor, final_dynamic_kelly) = if is_scalp {
            ...
            (vol_brake.max(vol_clamp_min), kelly * fraction_multiplier.max(1.0))
        } else {
        ...
        let final_leverage = capital_factor 
            * conviction 
            * final_vol_factor 
            * growth_factor 
            * hurst_bonus;
