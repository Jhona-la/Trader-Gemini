...
impl EpigeneticFitnessLandscapeEngine {
    ...
    pub fn compute_fitness_density(sharpe: f64, win_rate: f64, profit_factor: f64) -> f64 {
        ...
        let safe_sharpe = if sharpe.is_finite() {
            sharpe.max(0.0)
        } else {
            0.0
        };
        ...
        let safe_pf = if profit_factor.is_finite() && profit_factor > 1.0 {
            profit_factor - 1.0
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_epigenetic_fitness_density_nan_immunity() {
        let density =
            EpigeneticFitnessLandscapeEngine::compute_fitness_density(f64::NAN, f64::NAN, f64::NAN);
