...
impl MacroRegimeSwingOptimizer {
    ...
    pub fn evaluate_swing_regime(
        ...
        let safe_cvd = if cvd_imbalance.is_finite() {
            cvd_imbalance
        } else {
            0.0
        };
        let safe_regime_ms = if regime_duration_ms.is_finite() && regime_duration_ms >= 0.0 {
            regime_duration_ms
        } else {
            60_000.0
        };
        ...
        let v_scale = arena
            ...
            .load(std::sync::atomic::Ordering::Relaxed);
        let min_cool_r = arena
            .config
            .macro_min_cooldown_ratio
            .load(std::sync::atomic::Ordering::Relaxed);
        let max_cool_r = arena
            .config
            .macro_max_cooldown_ratio
            .load(std::sync::atomic::Ordering::Relaxed);
        let cool_red_f = arena
            .config
        ...
        let cooldown_reduction =
            combined_confidence * momentum_confidence * (base_cooldown_f64 * cool_red_f);
        ...
        let final_cooldown = if raw_cooldown.is_finite() {
            raw_cooldown.clamp(10.0, 1000.0) as usize
        } else {
            60
        };
        ...
        let leverage_modifier = if is_active {
            ...
            if raw_mod.is_finite() {
                raw_mod.clamp(0.1, 5.0)
            } else {
                0.0
            }
        } else {
