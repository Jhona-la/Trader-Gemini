...
pub async fn run_bybit_ws(state: Arc<OmniState>, symbol: String) {
    ...
    loop {
        if let Ok(target_addr) = tokio::net::lookup_host((host, port)).await.and_then(|mut iter| iter.next().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "DNS"))) {
            if let Ok(tcp_stream) = tokio::net::TcpStream::connect(target_addr).await {
                ...
                if let Ok((mut ws_stream, _)) = tokio_tungstenite::client_async_tls(url_parsed.clone(), tcp_stream).await {
            let msg = format!(r#"{{"op": "subscribe", "args": ["orderbook.1.{}"]}}"#, symbol);
            let _ = futures_util::SinkExt::send(&mut ws_stream, tokio_tungstenite::tungstenite::Message::Text(msg)).await;
    ...
    loop {
        if let Ok(target_addr) = tokio::net::lookup_host((host, port)).await.and_then(|mut iter| iter.next().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "DNS"))) {
            if let Ok(tcp_stream) = tokio::net::TcpStream::connect(target_addr).await {
                ...
                if let Ok((mut ws_stream, _)) = tokio_tungstenite::client_async_tls(url_parsed.clone(), tcp_stream).await {
            let msg = format!(r#"{{"op": "subscribe", "args": [{{"channel": "bbo-tbt", "instId": "{}"}}]}}"#, okx_symbol);
            let _ = futures_util::SinkExt::send(&mut ws_stream, tokio_tungstenite::tungstenite::Message::Text(msg)).await;
    ...
    pub fn start_feeds(&self, target_symbol: String) {
        ...
        tokio::spawn(async move {
            while let Some(res) = join_set.join_next().await {
                if let Err(e) = res {
                    println!("⚠️ [OMNI MULTIPLEXER] Tarea de alimentación de datos colapsada: {:?}", e);
                }
            }
