...
impl QuantumOscillatorEngine {
    ...
    pub fn compute_quantum_restoring_force(
        ...
        let safe_x = if position.is_finite() { position.clamp(-10.0, 10.0) } else { 0.0 };
        let safe_k = if k_spring.is_finite() && k_spring >= 0.0 { k_spring } else { 1.0 };
        let safe_l = if lambda_anharmonic.is_finite() && lambda_anharmonic >= 0.0 { lambda_anharmonic } else { 0.1 };
    ...
    pub fn compute_superposition_probability(position: f64, mass_freq_alpha: f64) -> f64 {
        let safe_x = if position.is_finite() { position.clamp(-10.0, 10.0) } else { 0.0 };
        let safe_alpha = if mass_freq_alpha.is_finite() && mass_freq_alpha > 0.0 { mass_freq_alpha.clamp(0.01, 10.0) } else { 1.0 };
...
mod tests {
    ...
    fn test_quantum_oscillator_engine_evaluate_with_registry() {
        ...
        assert!(eval < 0.0, "Desviación positiva debe generar fuerza restauradora negativa");
