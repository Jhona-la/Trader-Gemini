...
impl QuantumStrategy for SwingConformalFilterEngine {
    ...
    fn evaluate_for_coin(&self, coin_id: usize, symbol: &str) -> f64 {
        ...
        let conformal_alpha = r
            ...
            .map(|p| p.get_value())
            .unwrap_or(0.10);
        let safe_p = if conformal_p.is_finite() {
            conformal_p
        } else {
            0.95
        };
        let safe_alpha = if conformal_alpha.is_finite() {
            conformal_alpha
        } else {
            0.10
        };
...
mod tests {
    ...
    fn test_swing_conformal_filter() {
        ...
        let is_valid =
            SwingConformalFilterEngine::is_swing_confluence_valid(&arena, -2.0, 1.0, 0.99, true);
        assert!(is_valid);
    }
    ...
    fn test_swing_conformal_filter_short_and_rejection_cases() {
        ...
        let is_valid_short =
            SwingConformalFilterEngine::is_swing_confluence_valid(&arena, 2.0, -1.0, 0.99, false);
