...
impl TensorVoteOrchestrator {
    ...
    pub fn evaluate_consensus(&self) -> TensorDecision {
        if self.strategies.is_empty() {
            return TensorDecision { signal: SignalType::Flat, net_confidence: 0.0, expected_volatility: 0.0, expected_lifetime_ms: 0 };
        }
        ...
        let vol_scaler = self.config.tensor_dropout_rate.load(std::sync::atomic::Ordering::Relaxed).max(0.01);
        let expected_volatility = max_volatility * vol_scaler; 
        
        let confidence_cutoff = self.config.explosive_confidence_threshold.load(std::sync::atomic::Ordering::Relaxed);
        let base_duration = self.config.base_duration_ms.load(std::sync::atomic::Ordering::Relaxed) as u64;
        let expected_lifetime_ms = if net_confidence.abs() > confidence_cutoff { base_duration / 4 } else { base_duration };

        let ml_long_thresh = self.config.ml_threshold_long.load(std::sync::atomic::Ordering::Relaxed);
        let ml_short_thresh = self.config.ml_threshold_short.load(std::sync::atomic::Ordering::Relaxed);
        ...
        if net_confidence > ml_long_thresh {
            TensorDecision { signal: SignalType::Long, net_confidence, expected_volatility, expected_lifetime_ms }
        } else if net_confidence < -ml_short_thresh {
            TensorDecision { signal: SignalType::Short, net_confidence: net_confidence.abs(), expected_volatility, expected_lifetime_ms }
        } else {
            // Estasis: El sistema no tiene la convicción matemática para arriesgar el capital
            TensorDecision { signal: SignalType::Flat, net_confidence: net_confidence.abs(), expected_volatility: 0.0, expected_lifetime_ms: 0 }
        }
    }
}
