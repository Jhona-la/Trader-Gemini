...
impl CmaEsOptimizer {
    pub fn new(dimension: usize, initial_sigma: f64, override_lambda: Option<usize>) -> Self {
        ...
        let c_mu = (2.0 * (mueff - 2.0 + 1.0 / mueff))
            / ((dimension as f64 + 2.0).powi(2) + mueff).min(1.0 - c_1);
    ...
    pub fn update(
        ...
        let h_sigma = if ps_norm / ((1.0 - (1.0 - c_sigma).powi(2)) * self.dimension as f64).sqrt()
        ...
        for d in 0..self.dimension {
            ...
            self.cov_matrix[d][d] = (1.0 - self.c_1 - self.c_mu) * self.cov_matrix[d][d]
                + self.c_1
                    * (self.p_c[d] * self.p_c[d]
                        + (1.0 - h_sigma) * self.c_c * (2.0 - self.c_c) * self.cov_matrix[d][d])
