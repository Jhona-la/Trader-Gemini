...
pub fn run_evolution_daemon(
    ...
    tokio::spawn(async move {
        ...
        if let Ok(champ_scalp) =
            dark_alpha_engine::MoENeatEngine::load("config_dir/genotypes/moe_champion.bin")
        {
            println!(
                "🧬 [EVOLUTION-ENGINE] 🚀 Genoma campeón Scalp cargado exitosamente desde disco al inicio."
            );
            scalp_moe.store(Arc::new(champ_scalp.clone()));

            if let Ok(champ_swing) = dark_alpha_engine::MoENeatEngine::load(
                "config_dir/genotypes/moe_champion_swing.bin",
            ) {
                println!(
                    "🧬 [EVOLUTION-ENGINE] 🚀 Genoma campeón Swing especializado cargado exitosamente desde disco al inicio."
                );
                swing_moe.store(Arc::new(champ_swing));
            } else {
                swing_moe.store(Arc::new(champ_scalp));
            }
        } else if let Ok(champ_swing) =
            dark_alpha_engine::MoENeatEngine::load("config_dir/genotypes/moe_champion_swing.bin")
        {
            println!(
                "🧬 [EVOLUTION-ENGINE] 🚀 Genoma campeón Swing cargado exitosamente al inicio."
            );
            swing_moe.store(Arc::new(champ_swing));
        }
        ...
        loop {
            ...
            println!(
                "🧬 [EVOLUTION-ENGINE] Analizando entropía del modelo (Walk-Forward / Online Learning)..."
            );
            ...
            let scalp_needs_hyper =
                drawdown > panic_threshold || should_explore(real_scalp_wr, real_scalp_trades);
            {
                let mode = if scalp_needs_hyper {
                    "Hiper-Mutación"
                } else {
                    "Refinamiento"
                };
                println!(
                    "🧬 [EVOLUTION-ENGINE] Scalp: {} proactivo. WR: {:.1}%, DD: {:.2}%",
                    mode,
                    real_scalp_wr * 100.0,
                    drawdown * 100.0
                );
                ...
                let (real_kurtosis, real_skewness) =
                    estimate_distribution_shape(real_scalp_pf, real_scalp_wr);
                ...
                let penalized_pnl = EntropyFitness::reality_slippage_penalty(
                    real_scalp_pnl,
                    real_scalp_trades,
                    actual_fee,
                );
                ...
                if is_valid && deflated_sharpe > 0.5 {
                    ...
                    {
                        println!(
                            "⚠️ [EVOLUTION-ENGINE] Error guardando scalp champion: {}",
                            e
                        );
                    }

                    println!(
                        "🧬 [EVOLUTION-ENGINE] ✅ Hot-Swap Scalp MOE. DSR: {:.2}, WR: {:.1}%, BayesPenalty: {:.2}, PF: {:.2}, Trades: {}",
                        deflated_sharpe,
                        real_scalp_wr * 100.0,
                        bayesian_penalty,
                        real_scalp_pf,
                    ...
                    telemetry.record_tensor_telemetry(
                        1,
                        deflated_sharpe,
                        bayesian_penalty,
                        real_scalp_wr,
                    ...
                    println!(
                        "💀 [EVOLUTION-ENGINE] ❌ Scalp Mutación RECHAZADA. DSR: {:.2}, OOS Valid: {}, WR: {:.1}%",
                        deflated_sharpe,
                        is_valid,
                        real_scalp_wr * 100.0
                    );
            ...
            let swing_needs_hyper = drawdown > (panic_threshold * 1.5)
                || should_explore(real_swing_wr, real_swing_trades);
            {
                let mode = if swing_needs_hyper {
                    "Hiper-Mutación"
                } else {
                    "Refinamiento"
                };
                println!(
                    "🧬 [EVOLUTION-ENGINE] Swing: {} proactivo. WR: {:.1}%, DD: {:.2}%",
                    mode,
                    real_swing_wr * 100.0,
                    drawdown * 100.0
                );
                ...
                let (real_kurtosis, real_skewness) =
                    estimate_distribution_shape(real_swing_pf, real_swing_wr);
                ...
                // V13+FASE22 Reality Slippage Penalty (Fee real del Arena)
                let actual_fee = arena.config.max_fee_pct.load(Ordering::Relaxed).max(0.0001);
                let penalized_swing_pnl = EntropyFitness::reality_slippage_penalty(
                    real_swing_pnl,
                    real_swing_trades,
                    actual_fee,
                );
                ...
                if is_valid && deflated_sharpe > 0.5 {
                    ...
                    if let Err(e) =
                        new_swing.save_to_disk("config_dir/genotypes/moe_champion_swing.bin")
                    {
                        println!(
                            "⚠️ [EVOLUTION-ENGINE] Error guardando swing champion: {}",
                            e
                        );
                    }

                    println!(
                        "🧬 [EVOLUTION-ENGINE] ✅ Hot-Swap Swing MOE. DSR: {:.2}, WR: {:.1}%, BayesPenalty: {:.2}, PF: {:.2}, Trades: {}",
                        deflated_sharpe,
                        real_swing_wr * 100.0,
                        bayesian_penalty,
                        real_swing_pf,
                    ...
                    println!(
                        "💀 [EVOLUTION-ENGINE] ❌ Swing Mutación RECHAZADA. DSR: {:.2}, OOS Valid: {}, WR: {:.1}%",
                        deflated_sharpe,
                        is_valid,
                        real_swing_wr * 100.0
                    );
            ...
            if drawdown <= panic_threshold {
                println!(
                    "🧬 [EVOLUTION-ENGINE] Sistema estable. Micro-mutación de pesos completada. Capital: ${:.2}",
                    current_cap
                );
            }
        }
...
fn aggregate_real_stats(arena: &GlobalArena, _is_scalp: bool) -> (f64, usize, f64, f64) {
    ...
    let avg_wr = if total_trades > 0 {
        total_wins as f64 / total_trades as f64
    } else {
        0.0
    };
    let avg_pf = if pf_weight > 0.0 {
        weighted_pf_sum / pf_weight
    } else {
        1.0
    };
