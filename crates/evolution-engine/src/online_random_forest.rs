...
impl TrueOnlineRandomForest {
    ...
    pub fn retrain_models(&self) -> Result<(f64, f64), String> {
        ...
        let clf_params = RandomForestClassifierParameters::default().with_m(3).with_n_trees(20);
        ...
        let reg_params = RandomForestRegressorParameters::default().with_m(3).with_n_trees(20);
    ...
    pub fn predict_6d(&self, features: [f64; 6]) -> Option<(f64, f64)> {
        ...
        if let (Some(clf), Some(reg)) = (clf_guard.as_ref(), reg_guard.as_ref()) {
            ...
            let prob_win = clf.predict(&x_eval).unwrap_or_default().into_iter().next().unwrap_or(0) as f64;
            let expected_pnl = reg.predict(&x_eval).unwrap_or_default().into_iter().next().unwrap_or(0.0);
