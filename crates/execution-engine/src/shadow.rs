...
impl ShadowExecutor {
    ...
    fn log_shadow_trade(&self, symbol: &str, order_type: &str, is_long: bool, qty: f64, price: f64) {
        println!(
...
impl ExecutionProvider for ShadowExecutor {
    ...
    async fn execute_reduce_only_market(
        ...
        let side = if is_long_close { "SELL" } else { "BUY" };
        println!("👻 [SHADOW MODE] Reduce-Only Market {} {} {}", side, quantity, symbol);
        Ok(())
    }
...
mod tests {
    ...
    async fn test_shadow_executor_operations_and_nan_immunity() {
        ...
        // Valid execution
        let res_valid = shadow.execute_order(&order, "BTCUSDT", 50000.0, 0.001).await;
        ...
        let res_nan_qty = shadow.execute_raw_qty("BTCUSDT", true, f64::NAN, 0.001).await;
        ...
        assert!(shadow.execute_maker_chase("BTCUSDT", true, 0.001, 50000.0, 0.001, 0.1, "ID1").await.is_ok());
        assert!(shadow.execute_ioc_order("BTCUSDT", true, 0.001, 50000.0, 0.001, 0.1, "ID2").await.is_ok());
        assert!(shadow.execute_exchange_trailing_stop("BTCUSDT", true, 0.001, 50000.0, 1.0, 0.001, 0.1, "ID3").await.is_ok());
        assert!(shadow.execute_oco_order("BTCUSDT", true, 0.001, 51000.0, 49000.0, 0.001, 0.1, "ID4").await.is_ok());
        assert!(shadow.execute_iceberg_limit("BTCUSDT", true, 0.005, 0.001, 50000.0, 0.001, 0.1, "ID5").await.is_ok());
        assert!(shadow.execute_reduce_only_market("BTCUSDT", true, 0.001, 0.001).await.is_ok());
