        ...
        telemetry_server::profile_node!("GodEngineCore::process_event", {
            ...
            if is_trade {
                ...
                let min_atr_pct = self
                    ...
                    .dynamic_atr_min
                    .load(Ordering::Relaxed)
                    .max(0.001); // Force at least 0.1% floor
                ...
                if coin.positions.scalp_position.is_open() {
                    ...
                    let ticks_since_open = coin
                        .positions
                        .scalp_position
                        .trailing_phase
                        .load(Ordering::Relaxed) as u64; // reuse for tick count in phase 0
                    ...
                    if zombie_promote && !coin.positions.swing_position.is_open() {
                        ...
                        coin.positions.swing_position.trail_stop.store(
                            coin.positions
                                .scalp_position
                                .trail_stop
                                .load(Ordering::Relaxed),
                        ...
                        || zombie_close
                    {
                        ...
                        if is_win && pnl_pct.abs() > 0.0 {
                            ...
                            let new_pf =
                                old_pf + ((pnl_pct.abs() / old_pf.max(0.001) - old_pf) / n);
                            coin.scalp
                                .profit_factor
                                .store(new_pf.max(0.1), Ordering::Relaxed);
                        }
                ...
                if !coin.positions.scalp_position.is_open() && !latency_panic {
                    ...
                    let mut ml_prob = self
                        .scalp_forest
                        .as_ref()
                        .map(|f| f.predict(&features).unwrap_or(0.5) as f64)
                        .unwrap_or(0.5);
                    ...
                    if let Some(lh) = &self.lakehouse {
                        lh.record_tensor(
                            quantum_arena::symbol_registry::spec(coin_id).symbol,
                            event_time_ms,
                            features.to_vec(),
                            ml_prob as f32,
                            0.0,
                    ...
                    if atr_pct > 0.000001 {
                        if ml_prob > effective_threshold {
                            ...
                            let z_target = self
                                ...
                                .turbo_z_score_stdev
                                .load(Ordering::Relaxed);
                            scalp_intent = self.scalp_engines[coin_id].evaluate_microstructure(
                                bid_qty,
                                ask_qty,
                                z_target,
                                &self.arena,
            ...
            if is_kline_closed {
                if coin.positions.swing_position.is_open() {
                    ...
                    coin.positions
                        .swing_position
                        .trail_stop
                        .store(trail_res.stop_price, Ordering::Relaxed);
                    coin.positions
                        .swing_position
                        .max_pnl_pct
                        .store(trail_res.max_pnl_pct, Ordering::Relaxed);

                    let trail_hit = (is_long && current_price <= trail_res.stop_price)
                        || (!is_long
                            && current_price >= trail_res.stop_price
                            && trail_res.stop_price > 0.0);
                        ...
                        || swing_regime == MarketRegime::Neutral)
                {
                    ...
                    let nn_prob = if let Some(nn) = &mut self.swing_nn {
                        nn.predict(omni_features).unwrap_or(0.5)
                    } else {
                        0.5
                    };
                    ...
                    let swing_intent = self.swing_engines[coin_id].evaluate_trend(
                        current_price,
                        hurst_exponent,
                        trend_threshold,
                        nn_prob,
        ...
        telemetry_server::profile_node!("GodEngineCore::process_tick", {
            ...
            if coin.positions.swing_position.is_open() {
                ...
                let trail_res = crate::trailing::evaluate_quantum_trailing(
                    ...
                    2.0,
                    3.0,
                    0.7, // t_params for swing (pullback_tol as ATR scale)
                );
                ...
                let trail_hit = (is_long && mid_price <= trail_res.stop_price)
                    || (!is_long
                        && mid_price >= trail_res.stop_price
                        && trail_res.stop_price > 0.0);
            ...
            let total_vol = bid_qty + ask_qty;
            let current_obi = if total_vol > 0.0 {
                (bid_qty - ask_qty) / total_vol
            } else {
                0.0
            };
            ...
            if atr_pct > dynamic_atr_min {
                if (ml_prob - 0.5).abs() < 0.001 {
                    if current_obi > dynamic_obi_thr
                        && ema_trend > dynamic_ema_thr
                        && ofi > dynamic_ofi_thr
                    {
                        scalp_intent = SignalIntent {
                            signal: SignalType::Short,
                    ...
                    } else if current_obi < -dynamic_obi_thr
                        && ema_trend < -dynamic_ema_thr
                        && ofi < -dynamic_ofi_thr
                    {
                        scalp_intent = SignalIntent {
                    ...
                    if spot_bias > 0.1 || ml_prob > ml_threshold {
                        scalp_intent = SignalIntent {
                            signal: SignalType::Long,
                            confidence: ml_prob.max(0.85),
                            ..Default::default()
                        };
                    } else if spot_bias < -0.1 || ml_prob < (1.0 - ml_threshold) {
                        scalp_intent = SignalIntent {
                            signal: SignalType::Short,
                            confidence: (1.0 - ml_prob).max(0.85),
                            ..Default::default()
                        };
                    }
                }
            ...
            if scalp_order.signal != SignalType::Flat || swing_order.signal != SignalType::Flat {
                    ...
                    && scalp_order.signal != SignalType::Flat
                {
                    ...
                    let mut margin_required =
                        current_cap.max(0.0) * cap_split * adjusted_kelly * scalp_intent.confidence;
                    let max_position_size = 10000.0;
                    if margin_required * leverage > max_position_size {
                        margin_required = max_position_size / leverage;
                    }
