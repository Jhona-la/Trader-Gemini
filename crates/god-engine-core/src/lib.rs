...
impl GodEngineCore {
    ...
    pub fn process_tick_dual(
        ...
        telemetry_server::profile_node!("GodEngineCore::process_tick_dual", {
            ...
            // --- 1. GESTIÓN MULTI-HORIZONTE CONTINUA INTEGRAL POR RANURAS ARMÓNICAS ---
            for (slot_idx, pos) in coin.positions.slots().into_iter().enumerate() {
                ...
                // COTA INVIOLABLE ANTI-TRAMPA RETROACTIVA (#564):
                // be_stop SOLO se arma si el precio actual de mercado está EN GANANCIA por delante de be_stop.
                // Si el mercado ya retrocedió por detrás de be_stop, NO se arma retroactivamente para evitar una salida inmediata a peor precio.
                let price_ahead_of_be = if is_long {
                    mid_price > be_stop
                } else {
                    mid_price < be_stop
                };

                if be_triggered && price_ahead_of_be {
                ...
                if trail_active {
                    ...
                    let safe_stop = if is_long {
                        trail_res.stop_price.max(sl_floor).max(be_stop)
                    } else {
                        if trail_res.stop_price > 0.0 {
                            trail_res.stop_price.min(sl_floor).min(be_stop)
                        } else {
                            be_stop
                        }
                    };
                    ...
                    let final_stop = if is_long {
                        if current_stored > 0.0 {
                            safe_stop.max(current_stored).max(be_stop)
                        } else {
                            safe_stop.max(be_stop)
                        }
                    } else {
                        if current_stored > 0.0 {
                            safe_stop.min(current_stored).min(be_stop)
                        } else {
                            safe_stop.min(be_stop)
                        }
                    };

                    let final_stop_valid = if is_long {
                        final_stop < mid_price
                    } else {
                        final_stop > mid_price
                    };
                    if final_stop_valid {
                ...
                // D-492 / #548: toxic_cut_sl relativo al SL de la posición (75% del SL, o min 3.0x live fee)
                // Corta pérdidas tempranamente ante toxicidad de libro confirmada (VPIN > 0.85 y OFI adverso).
                let toxic_cut_sl = (sl * 0.75).max(live_fee * 3.0);
                ...
                // Cosecha ganancias significativas cuando el pico superó el 60% del TP (min 28 bps)
                    ...
                    || toxic_flow_exit
                {
                    ...
                    if self.diag_close_total < 100 {
                        println!(
                            "🚪 [CLOSE TRACE] #{} reason={} pnl_pct={:.4}% peak={:.4}% age={}s gross_pnl=${:.4} exit={:.2} entry={:.2} trail_stop={:.2} mid={:.2} h={:?} ts={}",
                    ...
                    coin.last_close_exit_price.store(exit_price, Ordering::Relaxed);
            ...
            if atr_pct > dynamic_atr_min
                && spread_ok
                && self.feature_engines[coin_id].can_open_at_tau(micro_tau, 60_000)
            {
                ...
                if self.arena.tick_counter.load(Ordering::Relaxed) % 50_000 == 0 {
                    telemetry_server::telemetry_log!(
                        "🔍 [DIAG] tick={} atr_pct={:.6} atr_min={:.6} spread_ok={} can_tau={} hurst={:.3} ema_s={:.2} mid={:.2} ml={:.3} book_absent={} obi={:.3} ofi={:.3} cvd={:.3}",
                ...
                if is_confirmed_downtrend {
                    ...
                    // F8-P9: Invariante de flujo y microtendencia — prohibido vender ante microtendencia adversa o flujo comprador
                    ...
                    // F8-P9: Invariante de flujo y microtendencia — prohibido comprar ante microtendencia adversa o flujo vendedor
                ...
                // Ruta Micro-Espectral Directa: resonancia armónica de alta frecuencia (tau = micro_tau)
                if fast_intent.signal == SignalType::Flat {
                    if let Some(spec) = self.temporal_spectrum.get(coin_id) {
                        let micro_sig = spec.signal_at(micro_tau);
                        let micro_pers = spec.persistence_at(micro_tau);
                        let micro_gain = spec.scale_gain_at(micro_tau);

                        if micro_sig.abs() > 0.40 && micro_pers > 0.48 {
                            let (dir, ok_ext, is_l) = if micro_sig > 0.0 {
                                (SignalType::Long, not_overextended_long, true)
                            } else {
                                (SignalType::Short, not_overextended_short, false)
                            };
                            let strk = if is_l { effective_long_streak } else { effective_short_streak };
                            if ok_ext && strk < 2 {
                                let conf = (0.58 + micro_sig.abs() * 0.22 * micro_gain).clamp(0.55, 0.94);
                                fast_intent = SignalIntent {
                                    signal: dir,
                                    confidence: conf,
                                    expected_duration_ms: fast_duration_ms,
            ...
            // Modo Espectral de Baja Frecuencia / Onda Macro (τ continuo sin piso discreto en 180s):
            let swing_duration_ms = (macro_tau.clamp(10_000.0, 86_400_000.0)).round() as u64;
            ...
            // El universo multivariante continuo temporal espectral de 32 partes reconoce que
            // ondas en escalas desacopladas (|Δ ln τ| >= 0.60) transportan energía de forma ortogonal.
            // Por lo tanto, no se canibalizan ni se fuerza un "winner-take-all":
            // - Si ambas bandas ven oportunidades en escalas ortogonales, ambas se preparan como candidatas
            //   para ser evaluadas y despachadas a sus respectivas ranuras armónicas independientes.
            // - Si compiten por la misma banda de frecuencia (|Δ ln τ| < 0.60), la interferencia destructiva
            //   se resuelve seleccionando la de mayor densidad de energía espectral viva.
            let mut candidates = [SignalIntent::flat(), SignalIntent::flat()];
            let mut num_candidates = 0;

            if fast_intent.signal != SignalType::Flat && slow_intent.signal != SignalType::Flat {
                let tau_f = if fast_intent.expected_duration_ms > 0 {
                    fast_intent.expected_duration_ms as f64
                } else {
                    micro_tau
                };
                let tau_s = if slow_intent.expected_duration_ms > 0 {
                    slow_intent.expected_duration_ms as f64
                } else {
                    macro_tau
                };
                ...
                if diff_ln >= 0.60 {
                    ...
                    candidates[0] = fast_intent;
                    candidates[1] = slow_intent;
                    num_candidates = 2;
                } else {
                    ...
                    if fast_intent.signal == slow_intent.signal {
                        ...
                        let (fast_energy, slow_energy) = self
                            ...
                            .map(|s| {
                                (
                                    s.continuous_energy_density(tau_f),
                                    s.continuous_energy_density(tau_s),
                                )
                            })
                            .unwrap_or((1.0, 1.0));
                        let best_duration = if fast_energy >= slow_energy {
                            fast_intent.expected_duration_ms
                        } else {
                        ...
                        let (fast_energy, slow_energy) = self
                            ...
                            .unwrap_or((1.0, 1.0));
                        candidates[0] = if fast_energy >= slow_energy {
                            fast_intent
                        } else {
                            slow_intent
                        };
                ...
                candidates[0] = slow_intent;
                num_candidates = 1;
            }

            let mut new_order = None;

            for c_idx in 0..num_candidates {
                let mut unified_intent = candidates[c_idx];
            ...
            self.diag_dir
                .funnel_begin(unified_intent.signal, !coin.positions.is_any_open());
            ...
            if unified_intent.signal != SignalType::Flat {
                ...
                if last_close > 0 {
                    ...
                    let whiplash_veto = if last_was_win {
                        ...
                        if is_same_dir {
                            let last_exit = coin.last_close_exit_price.load(Ordering::Relaxed);
                            let min_discount = 0.50 * cur_atr;
                            if elapsed_ms < 900_000 {
                                // Cooldown armónico incondicional: la onda necesita al menos 15 min para consolidar
                                true
                            } else if elapsed_ms < 3_600_000 {
                                // Dentro de 1 hora post-victoria: jamás comprar por encima o igual a la salida ganadora sin flujo institucional agresivo (cvd > 0.10)
                                if last_exit > 0.0 {
                                    if unified_intent.signal == SignalType::Long {
                                        mid_price >= (last_exit - min_discount)
                                            || (mid_price >= last_exit && rolling_cvd <= 0.10)
                                    } else {
                                        mid_price <= (last_exit + min_discount)
                                            || (mid_price <= last_exit && rolling_cvd >= -0.10)
                                    }
            ...
            // F8-P10: Invariante Universal de Flujo Institucional y Agresión de Mercado (CVD Universal Guard)
                        ...
                        if margin_req > 0.0 && margin_req <= free_cap {
                                ...
                                && total_used + margin_req <= current_cap * cushion
                            {
                                ...
                                if self.diag_opened <= 100 {
                                    println!(
                                        "🚀 [OPEN TRACE] #{} dir={} slot={} tau={}ms rama={:.0} h={:?} conf={:.4} lev={:.1}x margin=${:.2} notional=${:.2} sc={:.3} obi={:.3} ofi={:.3} cvd={:.3} ht={:.5} st={:.5} mac={:.5} mic={:.5} ts={}",
                                        self.diag_opened,
            ...
            if latency_ms <= 25 {
                ...
                for p in coin.positions.slots() {
                    if p.is_open() {
                        let is_long = p.is_long.load(Ordering::Relaxed);
                        let notional = p.quantity.load(Ordering::Relaxed) * mid_price;
