...
fn main() {
    ...
    println!(
        "[NANO-PROFILER] Evaluando Swing Strategy ({} iteraciones)...",
        iterations
    );
    ...
    println!(
        "  -> Swing Strategy: {} ciclos por evaluación",
        cycles_per_iter
    );
    // Asumiendo un CPU de ~3.5GHz, 1 ciclo = ~0.28 nanosegundos
    println!(
        "  -> Latencia Estimada (3.5 GHz): {:.2} nanosegundos",
        (cycles_per_iter as f64) * 0.285
    );

    // Benchmark Scalp Strategy
    println!(
        "\n[NANO-PROFILER] Evaluando Scalp Strategy ({} iteraciones)...",
        iterations
    );
    let start_cycles = rdtsc();
    for _ in 0..iterations {
    ...
    let cycles_per_iter = total_cycles / iterations;
    println!(
        "  -> Scalp Strategy: {} ciclos por evaluación",
        cycles_per_iter
    );
    println!(
        "  -> Latencia Estimada (3.5 GHz): {:.2} nanosegundos",
    ...
    println!(
        "  -> Costo de Heap Allocation: {} ciclos ({:.2} ns)",
        alloc_cycles_per_iter,
        (alloc_cycles_per_iter as f64) * 0.285
    );
