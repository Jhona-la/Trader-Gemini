...
fn simple_kline_to_ticks(coin_id: usize, kline: &Kline) -> Vec<TickEvent> {
    ...
    let total_v = if kline.volume.is_finite() && kline.volume > 0.0 {
        kline.volume
    } else {
        1.0
    };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut csv_file =
        std::fs::File::create("equity_curve.csv").expect("No se pudo crear equity_curve.csv");
    use std::io::Write;
    ...
    let requested_mutants: usize = std::env::args()
        .nth(3)
        .and_then(|s| s.parse().ok())
        .unwrap_or(250);
    ...
    let btc_path = {
        ...
        if real.exists() {
            println!(
                "📡 R2.2: usando AGGTRADES REALES (certificación sin artefactos de síntesis)."
            );
            real
        } else {
    ...
    let ticks = if is_multicoin {
        ...
        .collect();
        println!(
            "🌐 Modo Multi-Activo Habilitado: cargando {} archivos de ticks reales...",
            files.len()
        );
        backtest_engine::tick_replayer::load_multi_coin_binary_ticks(&files)?
    } else {
        unsafe {
            std::env::set_var("SINGLE_COIN_MODE", "1");
        }
        if !btc_path.exists() {
    ...
    println!(
        "✅ {} Ticks Reales cargados (AGGTRADES REALES) para Walk-Forward.",
        ticks.len()
    );
    ...
    let total_days_in_data =
        (((last_ts.saturating_sub(first_ts)) as f64 / ms_per_day as f64).ceil() as u64).max(1);
    let total_simulation_days = total_days_in_data.min(sim_days);

    let force_fresh = std::env::var("FORCE_FRESH_BASELINE")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false);
    let mut current_genome = if force_fresh {
        println!(
            "🧬 [BOOTSTRAP] FORCE_FRESH_BASELINE activo: iniciando desde Baseline Matemático."
        );
        ...
        quantum_arena::genome_store::GenomeEnvelope::load_active()
            ...
            .unwrap_or_else(|| {
                if let Ok(data) = std::fs::read_to_string("config_dir/genotypes/champion.json") {
                    ...
                    {
                        println!(
                            "🧬 [BOOTSTRAP] Cargado Genoma Campeón persistido desde champion.json."
                        );
                        return g;
                    }
                }
    ...
    println!(
        "📈 Iniciando simulación Walk-Forward de {} Días",
    ...
    while day_idx < total_simulation_days {
        ...
        println!(
            "🧬 [ENJAMBRE CUÁNTICO] Desplegando {} mutantes concurrentes en 10 nichos ecológicos...",
            num_mutants
        );
        for i in 0..num_mutants {
            ...
            mutant_arena
                .config
                .live_maker_fee
                .store(0.0002, Ordering::Relaxed);
            mutant_arena
                .config
                .live_taker_fee
                .store(0.0005, Ordering::Relaxed);
            mutant_arena
                .config
                .base_capital
                .store(initial_capital, Ordering::Relaxed);
            let mut mutant_genome = if i == 0 {
                ...
                g.explosive_confidence_threshold =
                    g.explosive_confidence_threshold.clamp(0.70, 0.88);
            ...
            mutant_genome.dynamic_obi_threshold =
                mutant_genome.dynamic_obi_threshold.clamp(0.15, 0.35);
        ...
        while idx < ticks.len() && ticks[idx].timestamp < day_end_ts {
            ...
            let is_kline_closed =
                prev_kline_ts == 0 || (tick.timestamp / 60_000) != (prev_kline_ts / 60_000);
            if is_kline_closed {
                prev_kline_ts = tick.timestamp;
            }
            ...
            // DIAGNÓSTICO SIGNAL-PATH: post-engine.
            {
                ...
                if wi.signal != SignalType::Flat {
                    diag.intents_swing += 1;
                }
                ...
                diag.max_obi = diag
                    .max_obi
                    .max(reg.get_value_or("orderbook_imbalance", 0.0).abs());
                diag.tech_thr = reg.get_value_or("tech_threshold", 0.0);
                diag.max_micro_trend = diag
                    .max_micro_trend
                    .max(reg.get_value_or("ema_trend", 0.0).abs());
                if new_order {
                    diag.orders += 1;
                }
            }
            ...
            shadow_engines.iter_mut().for_each(|shadow_engine| {
                shadow_engine.arena.update_market_data(
                    cid,
                    tick.bid_price,
                    tick.ask_price,
                    tick.bid_qty,
                ...
                shadow_engine.arena.coins[cid]
                    .current_price
                    .store(mid, Ordering::Relaxed);
        ...
        println!(
            "🏁 Loop terminado. Total ticks procesados en Día {}: {}",
            day_idx + 1,
            idx
        );
        ...
        println!(
            "📅 DÍA {}: Capital: ${:.4} (Efectivo: ${:.4}, Flotante: ${:+.4}) | PnL Día: {:+.4} ({:+.2}%) | Trades: {} (Continuous: {} [WR: {:.1}%, PnL: {:+.4}])",
            day_idx + 1,
            current_capital,
            day_final_cap,
            open_unrealized,
            ...
            day_trades,
            day_scalp_trades,
            if day_scalp_trades > 0 {
                (day_scalp_wins as f64 / day_scalp_trades as f64) * 100.0
            } else {
                0.0
            },
            day_scalp_pnl
        ...
        {
            let e = &engine;
            let wr = if e.diag_close_total > 0 {
                e.diag_close_wins as f64 / e.diag_close_total as f64
            } else {
                0.0
            };
            let avg_not = if e.diag_close_total > 0 {
                e.diag_notional_sum / e.diag_close_total as f64
            } else {
                0.0
            };
            let cap = e.arena.unified_capital.load(Ordering::Relaxed);
            println!(
                "🔬 [FILL-MODEL] closes={} WR={:.3} avg_notional=${:.2} max_notional=${:.2} capital=${:.2} avg_pnl={:.4}",
                e.diag_close_total,
                wr,
                avg_not,
                e.diag_notional_max,
                cap,
                if e.diag_close_total > 0 {
                    e.diag_pnl_sum / e.diag_close_total as f64
                } else {
                    0.0
                }
            );
        ...
        println!(
            ...
            diag.intents_swing,
            diag.orders,
            diag.max_intent_conf,
            diag.max_obi,
            diag.tech_thr,
            diag.max_micro_trend
        );
        // diag removed
        ...
        if all_mutant_stats.len() >= 5 {
            println!(
                "🧬 [POPULATION DIVERSITY] Top 5 Mutants: 1) #{}: PnL {:+.4} (Fit: {:+.4}, Trd: {}) | 2) #{}: {:+.4} | 3) #{}: {:+.4} | 4) #{}: {:+.4} | 5) #{}: {:+.4}",
                all_mutant_stats[0].0,
                all_mutant_stats[0].1,
                all_mutant_stats[0].2,
                all_mutant_stats[0].3,
                all_mutant_stats[1].0,
                all_mutant_stats[1].1,
                all_mutant_stats[2].0,
                all_mutant_stats[2].1,
                all_mutant_stats[3].0,
                all_mutant_stats[3].1,
                all_mutant_stats[4].0,
                all_mutant_stats[4].1
            );
        } else {
            println!(
                "🧬 [POPULATION DIVERSITY] Best Mutant: #{}: PnL {:+.4}",
                all_mutant_stats[0].0, all_mutant_stats[0].1
            );
        }
        ...
        for rank in 0..3.min(all_mutant_stats.len()) {
            ...
            println!(
                "   🏅 Rank #{}: Mutant #{} (PnL: {:+.4}, Trades: {}) -> tp: {:.4}, sl: {:.4}, tech_thr: {:.4}, kelly: {:.4}, trail_act: {:.4}",
                rank + 1,
                m_idx,
                safe_pnl,
                m_trd,
                ...
                g.scalp_sl_base,
                g.tech_threshold,
                g.scalp_kelly_fraction,
                g.scalp_trail_act_atr
            );
        }

        // PnL y Fitness del control (#0)
        let control_stats = all_mutant_stats
            .iter()
            .find(|(idx, _, _, _)| *idx == 0)
            .cloned()
            .unwrap_or((0, 0.0, 0.0, 0));
        let control_fitness = control_stats.2;
        let _control_trades = control_stats.3;
        let best_trades = all_mutant_stats
            .iter()
        ...
        let valid_replacement = (best_trades >= 1 && best_pnl > (control_pnl + 0.30))
            || (best_trades >= 1 && best_pnl > 0.0 && control_pnl <= 0.0); // R-04: sin actividad REAL mínima no hay reemplazo — ni la rama forzada promueve mutantes sin trades

        if valid_replacement {
            println!(
                "🧬 [EVOLUTION] Mutant #{} replaces Baseline! (PnL: {:.4} vs {:.4}, Imp: {:.4}, Trades: {})",
            ...
            println!(
                "🧬 [EVOLUTION] Mutante #{} superó al control en PnL ({:+.4} vs {:+.4}). Adoptando adaptación.",
                best_idx, best_pnl, control_pnl
            );
            current_genome = shadow_genomes[best_idx].clone();
        } else if best_pnl <= 0.0 && !(best_pnl > (control_pnl + 0.30)) {
            println!(
                "🧬 [ESTABILIDAD] Población en contracción sin mejora de capital. Genoma Base protegido."
            );
        } else {
            println!(
                "🧬 [ESTABILIDAD] El Genoma Base venció a la población mutante. Sin mutación."
            );
        }
        ...
        if !current_genome.dynamic_obi_threshold.is_finite() {
            current_genome.dynamic_obi_threshold = 0.25;
        }
        if !current_genome.dynamic_ofi_threshold.is_finite() {
            current_genome.dynamic_ofi_threshold = 0.25;
        }
        if !current_genome.dynamic_ema_trend.is_finite() {
            current_genome.dynamic_ema_trend = 0.00025;
        }

        current_genome.apply_to_arena(&arena);
        println!(
            "🧬 [GENOMA ACTUAL] tech_threshold: {:.6}, scalp_kelly_fraction: {:.6}, dynamic_obi: {:.4}",
            current_genome.tech_threshold,
            current_genome.scalp_kelly_fraction,
            current_genome.dynamic_obi_threshold
        );
