...
impl CoinArena {
    ...
    pub fn apply_epigenetic_feedback(&self, pnl_pct: f64, trade_duration_ms: u64) {
        ...
        self.epigenetic_threshold_modifier.store(new_threshold, Ordering::Relaxed);
    }
}
...
impl GlobalArena {
    ...
    #[inline(always)]
    pub fn update_market_data(&self, coin_id: usize, bid_price: f64, ask_price: f64, bid_qty: f64, ask_qty: f64, timestamp: u64) {
    ...
    pub fn update_agg_trade(&self, coin_id: usize, is_buyer_maker: bool, qty: f64) {
        if coin_id < 30 {
            ...
            self.coins[coin_id].agg_buy_vol.store(new_buy, Ordering::Relaxed);
            self.coins[coin_id].agg_sell_vol.store(new_sell, Ordering::Relaxed);
