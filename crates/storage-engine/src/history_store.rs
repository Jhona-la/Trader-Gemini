...
impl HistoryStore {
    ...
    pub fn insert_batch(&mut self, klines: &[KlineRow]) {
        let tx = self.conn.transaction().unwrap();
        {
            ...
            for k in klines {
                stmt.execute(params![k.coin_id as i64, k.timestamp as i64, k.open, k.high, k.low, k.close, k.volume]).unwrap();
            }
        }
        tx.commit().unwrap();
    }
    ...
    pub fn prune_older_than(&self, timestamp_ms: u64) {
        let _ = self.conn.execute("DELETE FROM klines WHERE timestamp < ?1", params![timestamp_ms as i64]);
    }
    ...
    pub fn get_recent_candles(&self, coin_id: usize, limit: usize) -> Vec<KlineRow> {
        ...
        let rows = stmt.query_map(params![coin_id as i64, limit as i64], |row| {
            Ok(KlineRow {
