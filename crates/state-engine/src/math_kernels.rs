...
impl WelfordVariance {
    ...
    pub fn update(&mut self, new_value: f64) {
        self.update_weighted(new_value, 1.0);
    }

    #[inline(always)]
    pub fn update_weighted(&mut self, new_value: f64, weight: f64) {
        if weight <= 0.0 { return; }
        if self.count >= 2000.0 {
            // Decaimiento exponencial para mantenerlo sensible como una ventana móvil
            self.m2 *= (self.count - 2.0 * weight) / (self.count - weight);
            self.count -= weight;
        }
        self.count += weight;
...
impl DynamicKelly {
    ...
    #[inline(always)]
    pub fn sizing_fraction(&self, current_capital: f64) -> f64 {
        ...
        // Asimetría por capital: 
        // Si el capital es pequeño (<$50), somos asimétricamente agresivos (hasta 2.0x Kelly).
        // Si el capital es grande (>$1000), nos volvemos defensivos (0.25x Kelly).
                // Asimetría por capital continua: Decaimiento matemático
        let base_reference = 50.0_f64;
        let capital_penalty = (base_reference / current_capital.max(10.0)).sqrt().clamp(0.1, 2.0);
        
        let adjusted_kelly = kelly * self.kelly_multiplier * capital_penalty;
        
        adjusted_kelly.clamp(0.01, 1.0)
    }
}

/// Shannon Entropy O(1) Algebraic Approximation for market noise measurement
#[derive(Debug, Clone)]
pub struct ShannonEntropy {
    bins: [f64; 10], // Simple 10-bin histogram approximation
    bin_contrib: [f64; 10], // Precomputed count * ln(count) contributions
    sum_contrib: f64, // Sum of all bin contributions
    total_count: f64,
}
...
impl ShannonEntropy {
    ...
    pub fn update(&mut self, norm_return: f64) -> f64 {
        ...
        let old_count = unsafe { *self.bins.get_unchecked(bin_idx) };
        let new_count = old_count + 1.0;
        unsafe { *self.bins.get_unchecked_mut(bin_idx) = new_count; }
        self.total_count += 1.0;
        
        let old_contrib = unsafe { *self.bin_contrib.get_unchecked(bin_idx) };
        // c * ln(c) -> 0 * ln(0) is mathematically 0.0
        let new_contrib = new_count * new_count.ln();
        self.sum_contrib = self.sum_contrib - old_contrib + new_contrib;
        unsafe { *self.bin_contrib.get_unchecked_mut(bin_idx) = new_contrib; }
        
        if self.total_count > 0.0 {
            // H = ln(N) - (1/N) * sum(c_i * ln(c_i))
            self.total_count.ln() - (self.sum_contrib / self.total_count)
        } else {
            0.0
        }
    }
...
impl RecursiveHurst {
    ...
    pub fn update(&mut self, price: f64) -> f64 {
        self.update_weighted(price, 1.0)
    }

    #[inline(always)]
    pub fn update_weighted(&mut self, price: f64, weight: f64) -> f64 {
        if weight <= 0.0 || price <= 0.0 { return self.current(); }
        ...
        // Fast Taylor/Simple-Return Approximation for log_return: ln(x) approx x - 1
        let log_return = (price - self.last_price) / self.last_price;
        self.last_price = price;
        
        let alpha = 0.005 * weight;
        self.mean_return = self.mean_return * (1.0 - alpha) + log_return * alpha;
        
        let deviation = log_return - self.mean_return;
        self.cum_dev = self.cum_dev * (1.0 - alpha) + deviation;
        
        self.max_p = self.max_p * (1.0 - alpha) + self.cum_dev * alpha;
        self.min_p = self.min_p * (1.0 - alpha) + self.cum_dev * alpha;
        
        if self.cum_dev > self.max_p { self.max_p = self.cum_dev; }
        if self.cum_dev < self.min_p { self.min_p = self.cum_dev; }
        
        self.std_dev.update_weighted(log_return, weight);
        
        if self.n >= 200.0 { self.n = 199.0; }
        self.n += weight.min(1.0);
        
        // Update cached Hurst value in update phase to avoid recalculations on hot reads
        let range = self.max_p - self.min_p;
        let std = self.std_dev.std_dev();
        self.cached_hurst = if std > 1e-12 && self.n > 10.0 {
            let rs = range / std;
            if rs > 0.0 {
    ...
    /// Returns the current Hurst exponent WITHOUT updating state or doing any logs.
    #[inline(always)]
...
#[inline(always)]
pub fn compute_kelly_fraction(p: f64, b: f64, apply_mult: bool, kelly_mult: f64, stress_score: f64, max_exposure: f64, current_capital: f64) -> f64 {
    if b <= 0.0 {
        return 0.0;
    }
    let q = 1.0 - p;
    ...
    // Capital adaptive modifier
            // Asimetría por capital continua: Decaimiento matemático
        let base_reference = 50.0_f64;
        let capital_penalty = (base_reference / current_capital.max(10.0)).sqrt().clamp(0.1, 2.0);
    
    let mut mult = kelly_mult * capital_penalty;
...
/// ⚡ ALGORITMO #13: SIMD VECTOR FEATURE EXTRACTOR ENGINE
/// Vectoriza el cálculo continuo de 4 activos simultáneamente en SIMD f64x4 
/// para reducir la latencia de procesamiento a nanosegundos puros.
#[derive(Debug, Clone, Copy, Default)]
#[repr(C, align(64))]
pub struct SimdFeatureExtractorEngine;

impl SimdFeatureExtractorEngine {
    #[inline(always)]
    pub fn process_4x_simd(
        prices: [f64; 4],
        prev_prices: [f64; 4],
        volumes: [f64; 4],
    ) -> ([f64; 4], [f64; 4]) {
        let p_vec = Simd::from_array(prices);
        let prev_vec = Simd::from_array(prev_prices);
        let vol_vec = Simd::from_array(volumes);

        let delta_vec = p_vec - prev_vec;
        let returns_vec = delta_vec / prev_vec;

        let flow_density_vec = returns_vec.abs() * vol_vec;

        (returns_vec.to_array(), flow_density_vec.to_array())
    }
}
