...
async fn main() -> Result<(), String> {
    ...
    let initial_capital_str = env::var("INITIAL_CAPITAL").expect("❌ INITIAL_CAPITAL must be provided as environment variable");
    ...
    let mut population: Vec<SuperGenotype> = (0..pop_size).map(|_| {
        SuperGenotype::new_random()
    }).collect();
    ...
    for gen in 1..=generations {
            ...
            .map(|genome| {
                ...
                for tick in &master_stream {
                    ...
                    if let Some((_, pnl, qty)) = closed_sc {
                        ...
                        if pnl > 0.0 { wins += 1; }
                        if total_trades <= 10 {
                            println!("🔍 TRACE [Scalp {}]: PnL=${:.10}, qty={:.8}", total_trades, pnl, qty);
                        }
                    }
                    if let Some((_, pnl, qty)) = closed_sw {
                        ...
                        if pnl > 0.0 { wins += 1; }
                        if total_trades <= 10 {
                            println!("🔍 TRACE [Swing {}]: PnL=${:.10}, qty={:.8}", total_trades, pnl, qty);
                        }
                    }
                ...
                (genome.clone(), final_capital, total_trades, fitness, win_rate, max_drawdown)
            })
            .collect();
        ...
        println!("   Peor  G{}: Cap ${:.2} | Trades: {} | Fitness: {:.2}", 
            gen, worst_gen.1, worst_gen.2, worst_gen.3);
        
        // Key parameters of best genome
        println!("   🧬 ATR_min={:.5} OBI_thr={:.3} EMA_thr={:.5} OFI_thr={:.3}", 
            best_gen.0.dynamic_atr_min, best_gen.0.dynamic_obi_threshold,
            best_gen.0.dynamic_ema_trend, best_gen.0.dynamic_ofi_threshold);
        println!("   🧬 Scalp TP={:.3}% SL={:.3}% | Swing TP={:.2}% SL={:.2}% | Lev={:.1}x",
            best_gen.0.scalp_tp_base * 100.0, best_gen.0.scalp_sl_base * 100.0,
            best_gen.0.swing_tp_base * 100.0, best_gen.0.swing_sl_base * 100.0,
    ...
    if let Some((ref genome, best_cap, best_trades, best_fitness)) = best_all_time {
        ...
        println!("Capital Split Scalp: {:.1}%", genome.capital_split_scalp * 100.0);
        ...
        println!("Scalp TP: {:.3}% | SL: {:.3}% | RR: {:.1}:1", 
            genome.scalp_tp_base*100.0, genome.scalp_sl_base*100.0, 
            genome.scalp_tp_base / genome.scalp_sl_base.max(0.0001));
        println!("Swing TP: {:.3}% | SL: {:.3}% | RR: {:.1}:1", 
            genome.swing_tp_base*100.0, genome.swing_sl_base*100.0, 
            genome.swing_tp_base / genome.swing_sl_base.max(0.0001));
        println!("Trend Threshold: {:.3}", genome.trend_threshold);
        println!("CVD Veto: {:.3} | Wall Veto: {:.2}", genome.cvd_veto_threshold, genome.wall_veto_threshold);
        println!("Trades: {}", best_trades);
        println!("Fitness: {:.4}", best_fitness);
        println!("Capital Final: ${:.2} ({:+.2}% Crecimiento)", best_cap, pnl_pct);
