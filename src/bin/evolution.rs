use quantum_engine::unified_engine::{run_backtest_native, ffi_run_unified_backtest, UnifiedConfig};
...
fn main() {
    ...
    for i in 0..iterations {
        ...
        test_cfg.sl_pct += random_f64(-0.005, 0.005) * temp / initial_temp;
        test_cfg.tp_pct += random_f64(-0.01, 0.01) * temp / initial_temp;
        test_cfg.ml_threshold_l += random_f64(-0.1, 0.1) * temp / initial_temp;
        test_cfg.ml_threshold_s += random_f64(-0.1, 0.1) * temp / initial_temp;
        test_cfg.tech_threshold_l += random_f64(-0.001, 0.001) * temp / initial_temp;
        test_cfg.tech_threshold_s += random_f64(-0.001, 0.001) * temp / initial_temp;
        
        // Constraints (Forcing Risk-Taking)
        if test_cfg.sl_pct < 0.001 { test_cfg.sl_pct = 0.001; }
        if test_cfg.tp_pct < 0.002 { test_cfg.tp_pct = 0.002; }
        if test_cfg.ml_threshold_l > 0.70 { test_cfg.ml_threshold_l = 0.70; } // Force trades
        if test_cfg.ml_threshold_l < 0.4 { test_cfg.ml_threshold_l = 0.4; }
        if test_cfg.ml_threshold_s > 0.70 { test_cfg.ml_threshold_s = 0.70; } // Force trades
        ...
        // Exponential Reward for > 1.0x compound rate
        let mut score = compound_rate_3d.powf(2.0) * 10000.0;
        
        // Regularity Penalties
        if trades < (days_simulated * 10.0) { score -= 10000.0; } // Force at least 10 trades per day
        
        // Master Rule Penalties
        if capital < 13.0 { score -= 100000.0; } // Never lose the starting 13
        if dd > 0.10 { score -= dd * 50000.0; } // Give up to 10% drawdown leeway for compounding
    ...
    let test_closes = unsafe { std::slice::from_raw_parts(ptr.add(len + train_len), test_len) };
    let test_highs = test_closes;
    let test_lows = test_closes;
    let test_volumes = unsafe { std::slice::from_raw_parts(ptr.add(len * 2 + train_len), test_len) };
    ...
    let compound_test = if oos_capital > 0.0 && periods_test > 0.0 {
        (oos_capital / 13.0).powf(1.0 / periods_test)
    } else {
        0.0
    };
