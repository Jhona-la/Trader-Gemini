...
pub use crate::telemetry::flight_recorder::{FlightRecorder, FlightEvent};
pub use crate::telemetry::telegram_bot::TelegramBot;
...
pub async fn start_telemetry_server(arena: Arc<GlobalArena>, tx: tokio::sync::broadcast::Sender<crate::dashboard::TelemetryEvent>) {
    crate::telemetry::profiler::start_profiler_auditor();
...
async fn get_tensor(
    ...
    let tensor = if coin_id < arena.coins.len() {
        arena.coins[coin_id].memory_matrix.extract_omni_tensor(16).to_vec()
    } else {
        vec![]
    };
    Json(TensorResponse {
...
async fn get_genome(State(arena): State<Arc<GlobalArena>>) -> Json<SuperGenotype> {
    let genome = SuperGenotype::current_from_arena(&arena);
    Json(genome)
}

async fn ws_handler(
    ws: WebSocketUpgrade,
    Extension(tx): Extension<tokio::sync::broadcast::Sender<crate::dashboard::TelemetryEvent>>,
) -> impl IntoResponse {
    ws.on_upgrade(move |socket| handle_socket(socket, tx))
}

async fn handle_socket(mut socket: WebSocket, tx: tokio::sync::broadcast::Sender<crate::dashboard::TelemetryEvent>) {
...
async fn get_state(State(arena): State<Arc<GlobalArena>>) -> Json<SystemState> {
    ...
    let mut win_rate_sum_scalp = 0.0;
    let mut win_rate_sum_swing = 0.0;
    let mut ml_prob_sum = 0.0;
    let mut hurst_sum = 0.0;
    let mut total_zombies = 0;
    let mut active_coins_scalp = 0.0;
    let mut active_coins_swing = 0.0;
    ...
    for coin in arena.coins.iter() {
        ...
        if sc_realized != 0.0 || sc_unrealized != 0.0 {
            win_rate_sum_scalp += wr_scalp;
            active_coins_scalp += 1.0;
        }
        if sw_realized != 0.0 || sw_unrealized != 0.0 {
            win_rate_sum_swing += wr_swing;
            active_coins_swing += 1.0;
        }
    }
    
    let avg_win_rate_scalp = if win_rate_sum_scalp > 0.0 { win_rate_sum_scalp / active_coins_scalp } else { 0.55 };
    let avg_win_rate_swing = if win_rate_sum_swing > 0.0 { win_rate_sum_swing / active_coins_swing } else { 0.55 };
    let avg_ml_prob = if !arena.coins.is_empty() { ml_prob_sum / (arena.coins.len() as f64) } else { 0.5 };
    let avg_hurst = if !arena.coins.is_empty() { hurst_sum / (arena.coins.len() as f64) } else { 0.5 };

    let total_fees = arena.total_fees_paid.load(Ordering::Relaxed);
    let pnl_pre_fees = pnl_realized_scalp + pnl_realized_swing + pnl_unrealized_scalp + pnl_unrealized_swing + total_fees;
    let pnl_post_fees = pnl_pre_fees - total_fees;
    
    let initial = arena.config.base_capital.load(Ordering::Relaxed);
    let roi_pre_fees = if initial > 0.0 { (pnl_pre_fees / initial) * 100.0 } else { 0.0 };
    let roi_post_fees = if initial > 0.0 { (pnl_post_fees / initial) * 100.0 } else { 0.0 };
    ...
    let state = SystemState {
        ...
        effective_max_leverage: {
            let cap = arena.unified_capital.load(Ordering::Relaxed).max(1.0);
            let size_penalty = (cap / 1_000.0).tanh();
            let fraction_multiplier = 0.8 - (size_penalty * 0.7);
            arena.config.global_leverage.load(Ordering::Relaxed) * fraction_multiplier
        },
...
<body>
    ...
    <div class="grid">
        ...
        <div class="card">
            <div class="card-title">🎯 Tasa de Aciertos Global</div>
    ...
    <script>
        ...
        const updateData = async () => {
            try {
                ...
                setHtml('val-win-rate', `<span class="${wrClass}">${formatNumber(data.win_rate_scalp * 100, 1)}</span> <span class="card-unit">%</span>`);
                ...
                setHtml('val-leverage', `${formatNumber(data.global_leverage, 0)} / ${formatNumber(data.effective_max_leverage, 1)} <span class="card-unit">x</span>`);
