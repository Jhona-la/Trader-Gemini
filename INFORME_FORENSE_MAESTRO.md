...
graph TD
    subgraph Root["Nodos Raíz (Data Source)"]
        WS["Binance WS Streams"]
        NTP["NTP Sync Engine"]
        REST["Binance REST API"]
        LAKE["Lakehouse Mmap / Ticks"]
    end

    subgraph Transform["Nodos de Transformación"]
        PARSER["parsers.rs / TensorParser"]
        WELFORD["ChannelWelfordStats (54D)"]
        OFI["OFI / OBI Microstructure"]
        REGISTRY["OmniscientRegistry (SkipMap Global)"]
        HAWKES_REAL["HawkesProcessEngine (feature-engine)"]
    end

    subgraph Decision["Nodos de Decisión"]
        FOREST["NanoForest (Scalp RF)"]
        DARK["DarkAlphaEngine 54D (Swing MLP)"]
        CONSEJO["Consejo de 10 Seniors"]
        ONLINE_L["OnlineLearningModule (metacortex)"]
        STRAT["Scalp & Swing Engines"]
        TENSOR["TensorVoteOrchestrator"]
        COAXIAL["CoaxialBreakoutEngine"]
        SOLITON["SolitonWaveEngine"]
        PERCEPTRON["PerceptronGateEngine"]
        RISK["RiskEngine & Kelly Envelope"]
        MATRIX["QuantumLeverageMatrix"]
        SHADOW["ShadowForest (10 Universos)"]
    end

    subgraph Terminal["Nodos Terminales"]
        EXEC["OrderExecutor (REST)"]
        WSEXEC["WsExecutor (WS Orders)"]
        POS["Atomic Position Manager"]
        STORE["GenomeStore / Active Genome"]
    end

    %% Conexiones y Fallos Detectados en el Grafo
    WS -->|Bytes JSON| PARSER
    WS -.->|"Fallo Tipo 1: Current price 0.0 descarta depth L2 en vivo"| PARSER
    PARSER --> OFI
    OFI --> REGISTRY
    REGISTRY -.->|"Fallo Tipo 3: Claves globales sobreescritas entre 30 monedas"| TENSOR
    HAWKES_REAL -.->|"Fallo Tipo 1: Motor real desconectado; L896 usa fórmula fake"| REGISTRY
    PARSER --> WELFORD
    WELFORD --> DARK
    DARK -.->|"Fallo Tipo 2: Condición en L1082 bloquea 99.9% de ticks"| STRAT
    ONLINE_L -.->|"Fallo Tipo 2: Instanciado en self.online_learner pero nunca ejecutado"| STRAT
    CONSEJO -.->|"Fallo Tipo 2: 10 Seniors instanciados pero nunca consultados"| RISK
    FOREST -->|ml_prob| STRAT
    COAXIAL -.->|"Fallo Lógico: Compresión idénticamente 0.0 por cancelación algebraica"| TENSOR
    SOLITON -.->|"Fallo Lógico: Fase t=0, x=0 colapsa ecuación no-lineal a sign(v)*|amp|"| TENSOR
    PERCEPTRON -.->|"Fallo Tipo 1: update_weight nunca se llama; peso Hebbiano clavado en 1.0"| TENSOR
    STRAT --> TENSOR
    TENSOR --> RISK
    RISK --> MATRIX
    MATRIX -.->|"Fallo Tipo 2: evaluate_quantum_order hardcodea is_scalp=true y L1138 descarta leverage"| POS
    RISK --> EXEC
    EXEC -.->|"Fallo Tipo 1: WsExecutor sender=None, siempre cae a REST con latencia 100ms"| REST
    EXEC -->|HTTP REST 100ms| REST
    SHADOW -.->|"Fallo Tipo 1: Gate rechaza 100% de genomas ganadores (bounds disjuntos)"| STORE
    SHADOW -.->|"Fallo Tipo 3: Replant cada 3s en ruido micro-tick borra capital"| STORE
    POS -.->|"Fallo Tipo 1: OCO sin motor de cancelación remota deja stops huérfanos"| REST
```
