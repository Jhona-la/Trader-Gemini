...
from core.consensus_filter import ConsensusFilter
original_check = ConsensusFilter.check_signal

def kamikaze_check_signal(self, signal_event, portfolio, current_price, risk_manager=None, meta_coordinator=None):
    passed, reason = original_check(self, signal_event, portfolio, current_price, risk_manager, meta_coordinator)
    
    if not passed:
        if "FEE_DRAG" in reason or "REGIME" in reason or "VOLATILITY" in reason:
            return True, "KAMIKAZE_OVERRIDE"
    return passed, reason

ConsensusFilter.check_signal = kamikaze_check_signal

# --- Import and run God Mode Backtest ---
from scripts.run_god_mode_backtest import run_global_backtest

logging.getLogger("trader_gemini").setLevel(logging.ERROR)
