...
class SurvivalRules:
    ...
    MAX_SESSION_DRAWDOWN_PCT = 0.02     # 2%

    # Rule 2: Max loss per individual trade
    MAX_LOSS_PER_TRADE_PCT = 0.005      # 0.5%

    # Rule 3: Max simultaneous open positions
    MAX_OPEN_POSITIONS = 3

    # Rule 4: Max trades per day (anti-overtrading)
    ...
    BLOCK_ON_VOLATILITY_BURST = True

    # Rule 7: Minimum BTC correlation for normal operation
    MIN_BTC_CORRELATION = 0.30
    ...
    def run_monte_carlo(cls, state: AccountState, iterations=1000) -> float:
        ...
        win_rate = 0.55 if not state.volatility_burst_active else 0.45
        avg_win = state.total_capital * 0.01
        avg_loss = state.total_capital * 0.015
        ...
        return ruined_paths / iterations
    ...
    def evaluate_all(cls, order: OrderIntent, state: AccountState) -> List[RuleResult]:
        ...
        trade_risk_usd = order.quantity * order.price * 0.01  # Rough 1% SL assumption
        results.append(RuleResult(
            2, "MAX_LOSS_PER_TRADE",
            trade_risk_usd <= max_risk_usd,
            f"TradeRisk=${trade_risk_usd:.4f} (limit=${max_risk_usd:.4f})"
        ))
        ...
        results.append(RuleResult(
            ...
            state.trades_today < cls.MAX_TRADES_PER_DAY,
            f"Today={state.trades_today} (limit={cls.MAX_TRADES_PER_DAY})"
        ))
...
class SovereignRiskShield:
    ...
    def evaluate(self, order: OrderIntent, state: AccountState) -> ShieldVerdict:
        ...
        if latency_ms > 50.0 and order.side == "BUY": # Assuming BUY opens a new position for now
        ...
        if time.time() < self.halt_until:
            remaining = int(self.halt_until - time.time())
        ...
        if len(violations) == 0:
            ...
            logging.warning(f"🔴 SHIELD VERDICT: BLOCK — 1 rule violated. Order DESTROYED.")
            return ShieldVerdict.BLOCK

        elif len(violations) == 2:
            self.orders_blocked += 1
            self._log_violation(order, violations)
            self.halt_until = time.time() + 900  # 15 minutes
