...
class GodModeSurrogate:
    def __init__(self, symbol: str, closes_arr: np.ndarray, horizon: str, n_trials: int = 100):
        self.symbol = symbol
...
def main():
    ...
    parser.add_argument('--trials', type=int, default=100, help="Trials para la Fase 4 de validación F4")
    ...
    for symbol in TARGET_COINS:
        if symbol in all_data and not all_data[symbol].empty:
            ...
            for horizon in ['scalping', 'swing']:
                ...
                print("="*50)
                god = GodModeSurrogate(symbol, closes, horizon, n_trials=args.trials)
                god.phase_explore()
